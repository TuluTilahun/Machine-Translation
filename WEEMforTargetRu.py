...
for i in range(len(reftranslation)):
    ...
    #f.write("\nWEEM\ten-ru\tnewstest2017\tonline-F.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    #f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-ru\tnewstest2016\tonline-F.0\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.write(str(WEEMscore)+"\n")
    #f.close()
    
    #wmt15  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE>
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-ru\tnewstest2015\tonline-F.0\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.write(str(WEEMscore)+"\n")
    #f.close()
    
    #wmt19  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE> <ENSEMBLE>   <AVAILABLE>
    f=open("WEEM.seg.score","a+")
    f.write("\nWEEM\ten-ru\tnewstest2019\tonline-E.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\tnon-ensemble" + "\tno")
    f.close()
...
f.write("\nWEEM\tcs-ru\tnewstest2019\tonline-G.0\t" + str(resultantscore) + "\t" + "\tnon-ensemble" + "\tno")
f.close()

#wmt17
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-ru\tnewstest2017\tonline-F.0\t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
#f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-ru\tnewstest2016\tonline-F.0\t" + str(resultantscore))
#f.close()

#wmt15  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SYSTEM LEVEL SCORE>
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-ru\tnewstest2015\tonline-F.0\t" + str(resultantscore))
#f.close()
