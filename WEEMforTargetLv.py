...
for i in range(len(reftranslation)):
    ...
    f.write("\nWEEM\ten-lv\tnewstest2017\tPJATK.4744\t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-lv\tnewstest2016\tJAIST.4859\t" + str(i+1) + "\t" + str(semscore))
    #f.close()
...
f.write("\nWEEM\ten-lv\tnewstest2017\tPJATK.4744\t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-lv\tDAsysNews+newstest2016\tJAIST.4859\t" + str(resultantscore))
#f.close()
