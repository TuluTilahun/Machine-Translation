...
for i in range(len(reftranslation)):
    ...
    #f.write("\nWEEM\ten-tr\tnewstest2017\tJAIST.4858\t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    #f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-tr\tnewstest2016\tParFDA.4543\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.close()
    
    #wmt19  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE> <ENSEMBLE>   <AVAILABLE>
    f=open("WEEM.seg.score","a+")
    f.write("\nWEEM\tru-tr\tnewstest2019\tonline-E.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\tnon-ensemble" + "\tno")
    f.close()
...
f.write("\nWEEM\tcs-tr\tnewstest2019\tonline-G.0\t" + str(resultantscore) + "\t" + "\tnon-ensemble" + "\tno")
f.close()

#wmt17
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-tr\tnewstest2017\tJAIST.4858\t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
#f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-tr\tnewstest2016\tParFDA.4543\t" + str(resultantscore))
#f.close()
