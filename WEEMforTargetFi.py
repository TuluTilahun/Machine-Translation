...
for i in range(len(reftranslation)):
    ...
    #f.write("\nWEEM\ten-fi\tnewstest2017\tapertium-unconstrained.4769 \t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    #f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-fi\tnewstest2016\tUUT.4529\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.close()
    
    #wmt15  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE>
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-fi\tnewstest2015\tGF-Wide-coverage-system.4129\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.close()
    
    #wmt19  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE> <ENSEMBLE>   <AVAILABLE>
    f=open("WEEM.seg.score","a+")
    f.write("\nWEEM\tru-fi\tnewstest2019\tonline-E.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\tnon-ensemble" + "\tno")
    f.close()
...
f.write("\nWEEM\tcs-fi\tnewstest2019\tonline-G.0\t" + str(resultantscore) + "\t" + "\tnon-ensemble" + "\tno")
f.close()

#wmt17
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-fi\tnewstest2017\tapertium-unconstrained.4769 \t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
#f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-fi\tnewstest2016\tUUT.4529\t" + str(resultantscore))
#f.close()

#wmt15  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SYSTEM LEVEL SCORE>
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-fi\tnewstest2015\tGF-Wide-coverage-system.4129\t" + str(resultantscore))
#f.close()
