...
ref=load_doc("translationref.txt")
sys=load_doc("translationsys.txt")
analyzer = ChineseAnalyzer()
reftranslation=list()
systranslation=list()
for i in range(len(ref)):
    reft = analyzer.parse(ref[i]) #The default tokenizer is jieba but we can also use pynlpir's to tokenize like : analyzer.parse(ref[i], using=analyzer.tokenizer.pynlpir)
    syst = analyzer.parse(sys[i])
    #print(result.tokens())
    r=[r for r in reft.tokens() if r!=' ']
    reftranslation.append(r)
    s=[s for s in syst.tokens() if s!=' ']
    systranslation.append(s)

print("System translation: ")
...
filename = 'cc.zh.300.vec.txt'
#filename='modelref.bin'
model = gensim.models.KeyedVectors.load_word2vec_format(filename+".bin", binary=True)
...
for i in range(len(reftranslation)):
    ...
    #f.write("\nWEEM\ten-zh\tnewstest2017\tonline-F.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    #f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-zh\tnewstest2016\tJAIST.4859\t" + str(i+1) + "\t" + str(semscore))
    #f.close()
    
    #wmt19  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE> <ENSEMBLE>   <AVAILABLE>
    f=open("WEEM.seg.score","a+")
    f.write("\nWEEM\tru-zh\tnewstest2019\tonline-E.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\tnon-ensemble" + "\tno")
    f.close()
...
f.write("\nWEEM\tcs-zh\tnewstest2019\tonline-G.0\t" + str(resultantscore) + "\t" + "\tnon-ensemble" + "\tno")
f.close()


#wmt17
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-zh\tnewstest2017\tonline-F.0\t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
#f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-zh\tDAsysNews+newstest2016\tJAIST.4859\t" + str(resultantscore))
#f.close()
