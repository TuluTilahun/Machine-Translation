...
for i in range(len(reftranslation)):
    ...
    #f.write("\nWEEM\ten-pl\tnewstest2015\ttuning-task-bleu-MIRA-sparse\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.close()
    
    #wmt16
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-pl\tnewstest2016\ttuning-task-usaar-harmonic-mert-mira.0\t" + str(i+1) + "\t" + str(WEEMscore))
    #f.close()
    
    #wmt17
    #f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\ten-pl\tnewstest2017\tuedin-nmt.4360\t" + str(i+1) + "\t" + str(WEEMscore) + "\t" + "\tnon-ensemble" + "\tno" )
    #f.close()
    
    #wmt19  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SEGMENT NUMBER>   <SEGMENT SCORE> <ENSEMBLE>   <AVAILABLE>
    f=open("WEEM.seg.score","a+")
    #f.write("\nWEEM\tru-pl\tnewstest2019\tonline-E.0\t" + str(i+1) + "\t" + str(WEEMscore) + "\tnon-ensemble" + "\tno")
...
f.write("\nWEEM\ten-pl\tnewstest2019\tonline-G.0\t" + str(resultantscore) + "\t" + "\tnon-ensemble" + "\tno")
f.close()

#wmt15  <METRIC NAME>   <LANG-PAIR>   <TEST SET>   <SYSTEM>   <SYSTEM LEVEL SCORE>
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-pl\tnewstest2015\ttuning-task-bleu-MIRA-sparse\t" + str(resultantscore))
#f.close()

#wmt16
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-pl\tnewstest2016\ttuning-task-usaar-harmonic-mert-mira.0\t" + str(resultantscore))
#f.close()

#wmt17
#f=open("WEEM.sys.score","a+")
#f.write("\nWEEM\ten-pl\tnewstest2017\tuedin-nmt.4360\t" + str(resultantscore) + "\t" + str(int(start.timestamp())) + "\t" + str(int(end.timestamp())) + "\tnon-ensemble" + "\tno" + "\t"+str(elapsedtime.seconds))
#f.close()
